	...
	for (var key in moviesObj){
		...
		if (key!=='genres'){
			...
			moviesObj[key].results.forEach(function(movie, index){
				var posterPath = movie.poster_path === null ? 'http://localhost:1337/movli.com/images/movie-default.jpg' : 'http://localhost:1337/image.tmdb.org/t/p/w300' + movie.poster_path
					...
					moviesObj[key].results.forEach(function(movie, index){
						for(var prop in movie){
							if (movie[prop] === parseInt(clickedId) && !found) {
								...
								$("#overlay").hide();
								$('#background').css({
									'background' : 'url(http://localhost:1337/image.tmdb.org/t/p/w1280' + movie.backdrop_path+') no-repeat',
